...
def custom_openapi(app: FastAPI, input_schema: Dict) -> Dict[str, Any]:
    """
    Method used to extend openapi schema generated by FastAPI in order to add descriptions and summaries for existing endpoints.
    It takes in as a parameter instance of FastAPI and a dictionary from which descriptions are surfaced.
    Returns a property of FastAPI openapi_schema, which holds a value of a newly generated openapi schema
    """
    ...
    openapi_schema = get_openapi(title="", version="", description="", routes=app.routes, )
